...
from app.schemas.detective_set_schema import (
    addDetectiveToSetRequest, 
    addDetectiveToSetResponse
)
...
@router.post(
    "/{room_id}/add-to-set", 
    response_model=addDetectiveToSetResponse, 
    status_code=200
)
async def add_to_set(
    ...
    """
    Endpoint para agregar un detective a set.
    
    Valida la carta, actualiza CardsXGame, crea la acción PENDING en ActionsPerTurn,
    y emite eventos WebSocket notificando el inicio de la acción.
    """
    logger.info(f"🎯 POST /api/game/{room_id}/add-to-set - Player {request.owner}")
    ...
    try:
        ...
        logger.info(f"Detective added to set successfully. Action ID: {action_id}")
    ...
    response = addDetectiveToSetResponse(
        success=True,
        actionId=action_id,
